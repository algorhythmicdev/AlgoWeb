<script>
    ...
    const getFocusableMenuItems = () => {
        ...
        return /** @type {HTMLElement[]} */ (
            Array.from(group.querySelectorAll(focusableSelectors)).filter(
                (element) => {
                    if (!(element instanceof HTMLElement)) return false;
    ...
    afterUpdate(async () => {
        ...
        if ($navigation.isMenuOpen !== wasMenuOpen) {
            ...
            if ($navigation.isMenuOpen) {
                ...
                await tick();
                const focusableItems = getFocusableMenuItems();
                const primaryLink = focusableItems.find(
                    (element) =>
    ...
    <div class="container nav-surface">
        <div class="nav-leading">
            <div class="nav-window-controls" aria-hidden="true">
                <span class="nav-window-control nav-window-control--close"
                ></span>
                <span class="nav-window-control nav-window-control--minimise"
                ></span>
                <span class="nav-window-control nav-window-control--expand"
                ></span>
            </div>
        ...
        <div class="nav-groups">
            <GlassCard
                ...
                inert={isCompactViewport && !$navigation.isMenuOpen
                    ? true
                    : undefined}
                padding="sm"
            >
                ...
                <ul class="nav-list">
                    {#each mainNavigation as item}
                        <li
                            class={`nav-item${item.children ? " nav-item--group" : ""}`}
                        >
                            <a
                                href={item.href}
                                class="nav-link"
                                    ...
                                    {#each item.children as child}
                                        <a
                                            ...
                                            on:click={() =>
                                                navigation.closeMenu()}
                                        >
                                            <span class="nav-submenu__title"
                                                >{$_(child.label)}</span
                                            >
                                            {#if child.description}
                                                <span
                                                    class="nav-submenu__description"
                                                    >{$_(
                                                        child.description,
                                                    )}</span
                                                >
                ...
                <div
                    ...
                    role="group"
                    aria-labelledby="nav-mobile-controls-heading"
                >
                    <h2
                        id="nav-mobile-controls-heading"
                        class="nav-mobile-controls__label"
                    >
...
<style>
    .nav-shell {
        ...
        background: color-mix(
            in srgb,
            var(--glass-bg-lightest) 54%,
            transparent 46%
        );
        border-bottom: 1px solid
            color-mix(in srgb, var(--glass-border) 62%, transparent 38%);
    ...
    .nav-shell.nav-condensed {
        background: color-mix(
            in srgb,
            var(--glass-bg-light) 52%,
            transparent 48%
        );
        border-bottom-color: color-mix(
            in srgb,
            var(--glass-border-strong) 62%,
            transparent 38%
        );
    ...
    .nav-window-controls {
        ...
        background: color-mix(
            in srgb,
            var(--surface-chip-bg) 70%,
            transparent 30%
        );
        border: 1px solid
            color-mix(in srgb, var(--surface-chip-border) 60%, transparent 40%);
    ...
    .nav-window-control {
        ...
        box-shadow:
            inset 0 1px 2px rgba(var(--snow-rgb), 0.35),
    ...
    .nav-window-control--minimise {
        background: color-mix(
            in srgb,
            var(--signal-yellow) 86%,
            var(--snow) 14%
        );
        border-color: color-mix(
            in srgb,
            var(--signal-yellow) 58%,
            rgba(var(--ink-rgb), 0.12) 42%
        );
    }
    ...
    :global([data-base-theme="dark"]) .nav-links {
        ...
        --glass-card-border: color-mix(
            in srgb,
            rgba(var(--voyage-blue-rgb), 0.48) 62%,
    ...
    .nav-link.active {
        ...
        background: color-mix(
            in srgb,
            var(--surface-chip-bg) 82%,
            rgba(var(--voyage-blue-rgb), 0.18) 18%
        );
        border-color: color-mix(
            in srgb,
            var(--surface-chip-border) 62%,
            rgba(var(--voyage-blue-rgb), 0.2) 38%
        );
        box-shadow: var(--surface-chip-shadow);
    }
    ...
    :global(.nav-item--group .nav-submenu) {
        ...
        --glass-card-surface: color-mix(
            in srgb,
            var(--glass-bg-lighter) 88%,
            transparent 12%
        );
        --glass-card-border: color-mix(
            in srgb,
            var(--glass-border-strong) 72%,
            transparent 28%
        );
    ...
    .nav-submenu__link:hover {
        border-color: color-mix(
            in srgb,
            var(--surface-field-border) 68%,
            rgba(var(--aurora-purple-rgb), 0.28) 32%
        );
        ...
        50% {
            box-shadow:
                0 0 0 4px
                    color-mix(
                        in srgb,
                        var(--focus-ring-color, var(--voyage-blue)) 70%,
                        transparent 30%
                    ),
                var(--nav-focus-shadow-base, transparent);
        }
    ...
    @media (max-width: 960px) {
        ...
        .nav-mobile-header {
            ...
            border-bottom: 2px solid
                color-mix(in srgb, var(--border-strong) 85%, transparent 15%);
        }
